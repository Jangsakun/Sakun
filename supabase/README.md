...
`migrations/20260828100000_db_size_monitor.sql` 실행 완료. pg_cron 잡
`record-db-size-daily` 등록까지 확인했습니다. 아래는 재구축이 필요할 때의 절차입니다.
...
**현재는 pg_cron 만 씁니다.** `record-db-size-daily` 잡이 매일 UTC 01:00(KST 10:00)에
`record_db_size_snapshot()` 을 실행합니다.
...
`vercel.json` 의 HTTP 크론은 2026-09-15 에 **삭제했습니다.** pg_cron 과 중복인 데다
`CRON_SECRET` 이 없어 매일 500 을 반환하며 실패 로그만 쌓고 있었습니다.
(되돌리려면 커밋 `8a72ae8` 의 `vercel.json` 을 복구)

### HTTP 스케줄러로 되돌려야 한다면

`/api/cron/db-size-snapshot` 라우트는 그대로 남아 있습니다(수동 호출·백업용).

1. 환경변수 `CRON_SECRET` 을 아무 긴 랜덤 문자열로 설정 (로컬 `.env.local` + Vercel 양쪽)
2. `vercel.json` 을 복구하거나 외부 스케줄러가 매일 UTC 01:00 에 호출하게 설정
